...
use hatsu_apub::{links::{Emoji, EmojiIcon, Mention, Tag, Tags}, objects::Note, tests::test_asset};
...
fn test_parse_notes() -> Result<(), AppError> {
    ...
    assert_eq!(akkoma_note.inner().clone().tag.unwrap(), Tags::Tags(vec![
        Tag::Mention(Mention {
            href: Url::parse("https://hatsu-nightly-debug.hyp3r.link/users/kwaa-blog-next.deno.dev")?,
